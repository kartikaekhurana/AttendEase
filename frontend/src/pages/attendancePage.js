...
export class attendancePage extends Component {
	...
	fetchStudents = async () => {
		...
		const url = `https://attendease-e5bz.onrender.com/api/students?courseId=${courseId}`;
		fetch(url)
			.then((response) => {
			...
			if (this.webcamRef.current) {
				...
				try {
					const response = await fetch(
						"https://attendease-e5bz.onrender.com/api/attendance/recognize",
						{
							method: "POST",
	...
	handleSubmitAttendance = () => {
		...
		fetch("https://attendease-e5bz.onrender.com/api/attendance/submit", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({
				courseId,
